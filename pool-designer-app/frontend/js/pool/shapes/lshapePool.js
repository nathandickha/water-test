...
/* -------------------------------------------------------
  ...
  for (let i = 0; i < borderPts.length; i++) {
    const p0 = borderPts[i];
    const p1 = borderPts[(i + 1) % borderPts.length];
    const segment = p1.clone().sub(p0);
    const segmentLength = segment.length();
    if (segmentLength < 0.01) continue;
    // Every adjacent wall shares the same calculated corner line. This closes
    // both the external corners and the re-entrant L corner without the box
    // overlaps and gaps caused by independent half-width extensions.
    const wallGeo = createMiteredWallPrism(borderPts, i, 0, clampedDeep, wallThickness);
    if (!wallGeo) continue;
    ...
    wall.position.set(0, 0, -clampedDeep * 0.5);
  ...
  for (let i = 0; i < pts2D.length; i++) {
    ...
    const segmentLength = segment.length();
    if (segmentLength < 0.01) continue;
    const copingGeo = createMiteredWallPrism(pts2D, i, 0.05, copingDepth, wallThickness);
    ...
    copingMesh.position.set(0,0,copingDepth * 0.5 + zOffset);
